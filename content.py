...
def main_page():
    st.title("Streamlit ECharts 图表库演示")
    st.warning("在右侧选择api以及样式库")
    st.markdown("这里展示了使用 echarts 做各种深度学习类型的数据的图表，例如饼状图，柱状图，散点图，折线图等等。这是一个简单，易于使用的数据可视化工具，在页面的结尾还提供了源代码")
    ...
    with st.sidebar:
        ...
        selected_api = st.selectbox(
            label="选择你偏好的api Choose your preferred API:",
            options=api_options,
        )
    ...
    with st.expander("源代码Source Code"):
